...
fn fs_main(input: VertexOutput) -> @location(0) vec4<f32> {
    ...
    let sphere_r = 0.3;
    let sphere_col = vec3<f32>(0.0, 0.0, 0.0);
    ...
    var hit = ray_plane_intersect(ray, plane_point, plane_normal);
    ...
    hit = ray_sphere_intersection(ray, sphere_c, sphere_r);
    if (hit.has_hit) {
      color   = hit.color;
      ray.tmax = hit.dist;
    }

    hit = ray_triangle_intersection(ray, triangle_v0, triangle_v1, triangle_v2);
    if (hit.has_hit) {
       color = hit.color;
       ray.tmax = hit.dist;
    }
...
fn ray_plane_intersect(ray: Ray, planePoint: vec3f, planeNormal: vec3f) -> HitInfo {
    ...
    if (abs(denom) > 1e-4) {
        ...
        if (t >= ray.tmin && t <= ray.tmax) {
            ...
            let color = vec3f(0.1, 0.7, 0.0); // your plane color
...
fn ray_triangle_intersection(ray: Ray, v0: vec3<f32>, v1: vec3<f32>, v2: vec3<f32>) -> HitInfo {
    let edge1 = v1 - v0;
    let edge2 = v2 - v0;
