...
fn fs_main(input: VertexOutput) -> @location(0) vec4<f32> {
    ...
    var hitInfo = HitInfo(false, 0.0, vec3f(0.0), vec3f(0.0), color, 0u);
...
fn intersect_scene(ray: ptr<function, Ray>, hitInfo: ptr<function, HitInfo>) -> bool {
    ...
    var found = false;
    let color = Color(vec3f(0.0), vec3f(0.0), vec3f(0.0));
    var best = HitInfo(false, 0.0, vec3f(0.0), vec3f(0.0), color, 0u);
...
fn shade(ray: ptr<function, Ray>, hitInfo: ptr<function, HitInfo>) -> vec3f {
    ...
    var shadowHitInfo = HitInfo(false, 0.0, vec3f(0.0), vec3f(0.0), Color(vec3f(0.0), vec3f(0.0), vec3f(0.0)), 0u);
...
fn ray_plane_intersect(ray: Ray, planePoint: vec3f, planeNormal: vec3f, color: vec3f) -> HitInfo {
    ...
    if (abs(denom) > 1e-4) {
        ...
        if (t >= ray.tmin && t <= ray.tmax) {
            ...
            return HitInfo(true, t, position, normal, randoColor, 0u);
        }
    }
    return HitInfo(false, 0.0, vec3f(0.0), vec3f(0.0), Color(vec3f(0.0), vec3f(0.0), vec3f(0.0)), 0u);
}
...
fn ray_triangle_intersection(ray: Ray, v0: vec3f, v1: vec3f, v2: vec3f, color: vec3f) -> HitInfo {
    ...
    if (t > ray.tmin && t < ray.tmax) {
        ...
        return HitInfo(true, t, position, normal, colorAsDiffuse, 2u);
    }
    return HitInfo(false, 0.0, vec3f(0.0), vec3f(0.0), Color(vec3f(0.0), vec3f(0.0), vec3f(0.0)), 0u);
}
...
fn ray_sphere_intersection(ray: Ray, sphereCenter: vec3f, sphereRadius: f32, color: vec3f) -> HitInfo {
    ...
    return HitInfo(true, t, position, normal, colorAsDiffuse, 1u);
}
