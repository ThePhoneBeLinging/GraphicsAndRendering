async function main() {
    ...
    const dollyModeBtn = document.getElementById('dolly-mode');
    const panModeBtn = document.getElementById('pan-mode');

    orbitModeBtn.addEventListener('click', () => interactionMode = 'orbit');
    dollyModeBtn.addEventListener('click', () => interactionMode = 'dolly');
    panModeBtn.addEventListener('click', () => interactionMode = 'pan');
    ...
    let spinQuaternion = new Quaternion(); // Identity by default
    let lookAtPoint = vec3(0, 0, 0);
    ...
    function projectToSphere(x, y) {
        const r2 = x * x + y * y;
        if (r2 < 1.0) {
            return vec3(x, y, Math.sqrt(1.0 - r2));
        } else {
            const r = Math.sqrt(r2);
            return vec3(x / r, y / r, 0.0);
        }
    }

    canvas.addEventListener('mousedown', (e) => {
        isMouseDown = true;
        spinQuaternion = new Quaternion(); // Reset spin on new interaction
        if (spinTimeout) {
            clearTimeout(spinTimeout);
            spinTimeout = null;
        }
        const x = (2.0 * e.clientX) / canvas.width - 1.0;
        const y = 1.0 - (2.0 * e.clientY) / canvas.height;
        lastPos = projectToSphere(x, y);
    });
    ...
    canvas.addEventListener('mouseup', () => {
        ...
        if (now - lastMoveTime > 20) {
            spinQuaternion = new Quaternion(); // Stop spinning
        } else {
    ...
    canvas.addEventListener('mousemove', (e) => {
        ...
        if (interactionMode === 'orbit') {
            const currentPos = projectToSphere(x, y);

            const dot_val = dot(lastPos, currentPos);
            if (Math.abs(dot_val) < 0.9999) {
                const axis = cross(lastPos, currentPos);
                const angle = Math.acos(dot_val);
                const delta = new Quaternion().make_rot_angle_axis(angle, axis);
                spinQuaternion = delta; // Store the incremental rotation
                rotation.multiply(delta);
            }
            lastPos = currentPos;
        } else {
            const currentPos = vec2(x, y);
            const lastPos2D = vec2(lastPos.x, lastPos.y);
            const delta = subtract(currentPos, lastPos2D);

            if (interactionMode === 'dolly') {
                radius -= delta[1] * 5; // Adjust sensitivity
                radius = Math.max(0.1, radius);
            } else if (interactionMode === 'pan') {
                const eye = add(lookAtPoint, rotation.apply(vec3(0, 0, radius)));
                const at = lookAtPoint;
                const up = vec3(0, 1, 0);
                const viewMat = lookAt(eye, at, up);
                const right = vec3(viewMat[0][0], viewMat[1][0], viewMat[2][0]);
                const viewUp = vec3(viewMat[0][1], viewMat[1][1], viewMat[2][1]);

                const panSpeed = 0.01 * radius;
                lookAtPoint = add(lookAtPoint, scale(-delta[0] * panSpeed, right));
                lookAtPoint = add(lookAtPoint, scale(delta[1] * panSpeed, viewUp));
            }
            lastPos = projectToSphere(x, y);
        }
    ...
    const normalmap_img = await (async () => {
        const resp = await fetch('./normalmap.png');
        if (!resp.ok) throw new Error('failed to load normalmap');
        const blob = await resp.blob();
        return await createImageBitmap(blob);
    })();

    const normalTexture = device.createTexture({
        size: [normalmap_img.width, normalmap_img.height, 1],
        format: 'rgba8unorm',
        usage: GPUTextureUsage.TEXTURE_BINDING | GPUTextureUsage.COPY_DST | GPUTextureUsage.RENDER_ATTACHMENT,
    });
    device.queue.copyExternalImageToTexture(
        { source: normalmap_img, flipY: true },
        { texture: normalTexture, origin: [0, 0, 0] },
        [normalmap_img.width, normalmap_img.height]
    );
    ...
    animate = () => {
        if (!orbiting) return;
        if (!isMouseDown) {
            rotation.multiply(spinQuaternion);
        }
        updateAndRender();
