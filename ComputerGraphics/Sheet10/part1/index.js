async function main() {
    ...
    canvas.addEventListener('mousemove', (e) => {
        ...
        angleAlpha += deltaX * 0.01;
        angleBeta += deltaY * 0.01;

        angleBeta = Math.max(-Math.PI / 2, Math.min(Math.PI / 2, angleBeta));
    ...
    updateAndRender = () => {
        const eye = vec3(
            radius * Math.cos(angleBeta) * Math.sin(angleAlpha),
            radius * Math.sin(angleBeta),
            radius * Math.cos(angleBeta) * Math.cos(angleAlpha)
        );
        const at = vec3(0, 0, 0);
        const up = vec3(0, 1, 0);
