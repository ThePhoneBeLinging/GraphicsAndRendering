async function main() {
    ...
    var loadedObject = await readOBJFile("suzanne.obj", 1.0, true);
    console.log("123");
    var positions = [];
    var indices = [];
    for (let i = 0; i < loadedObject.vertices.length; i += 4) {
        positions.push(loadedObject.vertices[i]);
        positions.push(loadedObject.vertices[i + 1]);
        positions.push(loadedObject.vertices[i + 2]);
        positions.push(loadedObject.normals[i]);
        positions.push(loadedObject.normals[i + 1]);
        positions.push(loadedObject.normals[i + 2]);
    }

    for (let i = 0; i < loadedObject.indices.length; i+= 3) {
        indices.push(loadedObject.indices[i]);
        indices.push(loadedObject.indices[i + 1]);
        indices.push(loadedObject.indices[i + 2]);
    }
